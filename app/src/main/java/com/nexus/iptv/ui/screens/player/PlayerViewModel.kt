...
import com.nexus.iptv.cast.CastConnectionState
import com.nexus.iptv.cast.CastManager
import com.nexus.iptv.cast.CastMediaRequest
import com.nexus.iptv.cast.CastStartResult
import com.nexus.iptv.di.MainPlayerEngine
import com.nexus.iptv.player.LivePreviewHandoffManager
import com.nexus.iptv.plugins.StreamVaultPluginManager
import com.nexus.iptv.util.isPlaybackComplete
import com.nexus.iptv.tv.LauncherRecommendationsManager
import com.nexus.iptv.tv.WatchNextManager
import com.nexus.iptv.data.remote.stalker.StalkerUrlFactory
import com.nexus.iptv.data.remote.xtream.XtreamStreamUrlResolver
import com.nexus.iptv.data.security.CredentialDecryptionException
...
import com.nexus.iptv.domain.model.CombinedCategory
import com.nexus.iptv.domain.model.CombinedM3uProfileMember
...
import com.nexus.iptv.domain.model.Favorite
import com.nexus.iptv.domain.model.LiveChannelObservedQuality
...
import com.nexus.iptv.domain.model.RecordingStatus
...
import com.nexus.iptv.domain.model.VideoFormat
import com.nexus.iptv.domain.usecase.GetCustomCategories
import com.nexus.iptv.domain.usecase.MarkAsWatched
import com.nexus.iptv.domain.usecase.ScheduleRecording
...
import com.nexus.iptv.domain.repository.EpgRepository
...
import com.nexus.iptv.player.Media3PlayerEngine
import com.nexus.iptv.player.AUDIO_VIDEO_OFFSET_MAX_MS
import com.nexus.iptv.player.AUDIO_VIDEO_OFFSET_MIN_MS
...
import com.nexus.iptv.player.PlayerSubtitleStyle
import com.nexus.iptv.player.timeshift.LiveTimeshiftBackend
import com.nexus.iptv.player.timeshift.LiveTimeshiftState
import com.nexus.iptv.player.timeshift.LiveTimeshiftStatus
import com.nexus.iptv.player.timeshift.TimeshiftConfig
...
class PlayerViewModel @Inject constructor(
    ...
    internal val favoriteRepository: com.nexus.iptv.domain.repository.FavoriteRepository,
    internal val playbackHistoryRepository: PlaybackHistoryRepository,
    internal val providerRepository: com.nexus.iptv.domain.repository.ProviderRepository,
    internal val combinedM3uRepository: CombinedM3uRepository,
    internal val preferencesRepository: com.nexus.iptv.data.preferences.PreferencesRepository,
    ...
    internal val currentChannelFlow = MutableStateFlow<com.nexus.iptv.domain.model.Channel?>(null)
    val currentChannel: StateFlow<com.nexus.iptv.domain.model.Channel?> = currentChannelFlow.asStateFlow()
    ...
    internal val currentChannelFlowList = MutableStateFlow<List<com.nexus.iptv.domain.model.Channel>>(emptyList())
    val currentChannelList: StateFlow<List<com.nexus.iptv.domain.model.Channel>> = currentChannelFlowList.asStateFlow()

    internal val recentChannelsFlow = MutableStateFlow<List<com.nexus.iptv.domain.model.Channel>>(emptyList())
    val recentChannels: StateFlow<List<com.nexus.iptv.domain.model.Channel>> = recentChannelsFlow.asStateFlow()
    ...
    private var preferredSurfaceMode: com.nexus.iptv.domain.model.PlayerSurfaceMode =
        com.nexus.iptv.domain.model.PlayerSurfaceMode.AUTO
     ...
     */
    internal var channelList: List<com.nexus.iptv.domain.model.Channel> = emptyList()
        set(value) {
            field = value
            rebuildChannelNumberIndex()
        }
    internal var channelNumberIndex: Map<Int, com.nexus.iptv.domain.model.Channel> = emptyMap()
        private set
    ...
    internal fun logRepositoryFailure(operation: String, result: com.nexus.iptv.domain.model.Result<Unit>) {
        if (result is com.nexus.iptv.domain.model.Result.Error) {
    ...
    val playerStats: StateFlow<com.nexus.iptv.player.PlayerStats> =
        activeEngineState(com.nexus.iptv.player.PlayerStats()) { it.playerStats }
    val availableAudioTracks: StateFlow<List<com.nexus.iptv.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableAudioTracks }
    }
    val availableSubtitleTracks: StateFlow<List<com.nexus.iptv.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableSubtitleTracks }
    }
    val availableVideoQualities: StateFlow<List<com.nexus.iptv.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
    ...
    internal suspend fun preparePlayer(
        streamInfo: com.nexus.iptv.domain.model.StreamInfo,
        requestVersion: Long
    ): Boolean {
    ...
    private suspend fun probePlaybackUrl(streamInfo: com.nexus.iptv.domain.model.StreamInfo): PlaybackProbeFailure? {
    ...
    private suspend fun shouldProbePlaybackUrl(url: String): Boolean {
        ...
        return (
            provider.type == com.nexus.iptv.domain.model.ProviderType.XTREAM_CODES ||
                provider.type == com.nexus.iptv.domain.model.ProviderType.STALKER_PORTAL
            ) &&
                    ...
                    _playerDiagnostics.update {
                        it.copy(
                            ...
                            providerSourceLabel = when (provider.type) {
                                com.nexus.iptv.domain.model.ProviderType.XTREAM_CODES -> "Xtream Codes"
                                com.nexus.iptv.domain.model.ProviderType.M3U -> "M3U Playlist"
                                com.nexus.iptv.domain.model.ProviderType.STALKER_PORTAL -> "Stalker/MAG Portal"
                            }
                        )
                    }
                }
    ...
    internal fun updateChannelDiagnostics(channel: com.nexus.iptv.domain.model.Channel) {
    ...
    internal suspend fun resolvePlaybackStreamInfo(
        ...
        providerId: Long,
        contentType: ContentType
    ): com.nexus.iptv.domain.model.StreamInfo? {
