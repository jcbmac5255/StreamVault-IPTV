...
import com.nexus.iptv.data.local.dao.CatalogSyncDao
import com.nexus.iptv.data.local.dao.CategoryDao
...
import com.nexus.iptv.data.local.dao.TmdbIdentityDao
import com.nexus.iptv.data.local.dao.XtreamContentIndexDao
import com.nexus.iptv.data.local.dao.XtreamIndexJobDao
import com.nexus.iptv.data.local.dao.XtreamLiveOnboardingDao
import com.nexus.iptv.data.local.entity.CategoryEntity
import com.nexus.iptv.data.local.entity.ChannelEntity
import com.nexus.iptv.data.local.entity.ChannelGuideSyncEntity
import com.nexus.iptv.data.local.entity.MovieEntity
import com.nexus.iptv.data.local.entity.SeriesEntity
import com.nexus.iptv.data.local.entity.XtreamContentIndexEntity
import com.nexus.iptv.data.local.entity.XtreamIndexJobEntity
import com.nexus.iptv.data.local.entity.XtreamLiveOnboardingStateEntity
import com.nexus.iptv.data.mapper.toDomain
import com.nexus.iptv.data.mapper.toEntity
import com.nexus.iptv.data.parser.M3uParser
import com.nexus.iptv.data.remote.http.buildAppRequestProfile
...
import com.nexus.iptv.data.remote.stalker.StalkerProviderProfile
import com.nexus.iptv.data.remote.dto.XtreamCategory
import com.nexus.iptv.data.remote.dto.XtreamSeriesItem
import com.nexus.iptv.data.remote.dto.XtreamStream
import com.nexus.iptv.data.remote.xtream.OkHttpXtreamApiService
import com.nexus.iptv.data.remote.xtream.XtreamAuthenticationException
import com.nexus.iptv.data.remote.xtream.XtreamNetworkException
import com.nexus.iptv.data.remote.xtream.XtreamParsingException
import com.nexus.iptv.data.remote.xtream.XtreamRequestException
import com.nexus.iptv.data.remote.xtream.XtreamResponseTooLargeException
import com.nexus.iptv.data.remote.xtream.XtreamApiService
import com.nexus.iptv.data.remote.xtream.XtreamProvider
import com.nexus.iptv.data.remote.xtream.XtreamUrlFactory
import com.nexus.iptv.data.security.CredentialCrypto
import com.nexus.iptv.data.util.AdultContentClassifier
import com.nexus.iptv.data.util.UrlSecurityPolicy
...
import com.nexus.iptv.domain.model.VodSyncMode
import com.nexus.iptv.domain.repository.EpgRepository
import com.nexus.iptv.domain.repository.EpgSourceRepository
...
class SyncManager @Inject constructor(
    ...
    private val preferencesRepository: com.nexus.iptv.data.preferences.PreferencesRepository,
    ...
    suspend fun syncEpg(
        ...
        try {
            ...
            com.nexus.iptv.domain.model.Result.success(Unit)
            ...
            com.nexus.iptv.domain.model.Result.error(safeMessage, e)
        }
    }
    ...
    suspend fun rebuildXtreamIndex(
        ...
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock com.nexus.iptv.domain.model.Result.error("Index rebuild is only available for Xtream providers")
        }
    ...
    private suspend fun syncXtreamIndexFirst(
        ...
        syncProgressBus.emit(
            com.nexus.iptv.domain.sync.SyncProgress(
                section = com.nexus.iptv.domain.sync.Section.VOD,
                current = 0,
                total = 0,
                currentLabel = "",
        ...
        syncProgressBus.emit(
            com.nexus.iptv.domain.sync.SyncProgress(
                section = com.nexus.iptv.domain.sync.Section.SERIES,
                current = 0,
                total = 0,
                currentLabel = "",
                itemsIndexed = liveCount
    ...
    suspend fun processQueuedXtreamIndexJobs(
        ...
        val providerEntity = providerDao.getById(providerId)
            ?: return@lock com.nexus.iptv.domain.model.Result.error("Provider $providerId not found")
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock com.nexus.iptv.domain.model.Result.success(Unit)
        }
        ...
        if (warnings.isNotEmpty()) {
            ...
            com.nexus.iptv.domain.model.Result.error(warnings.first(), exception)
        } else {
            com.nexus.iptv.domain.model.Result.success(Unit)
        }
    }
    ...
    private suspend fun syncStalkerPortalEpg(
        ...
        val insertBuffer = ArrayList<com.nexus.iptv.data.local.entity.ProgramEntity>(STALKER_GUIDE_PROGRAM_BATCH_SIZE)
        ...
        fallbackGuideRequests.forEachIndexed { index, request ->
            ...
            runCatching {
                ...
                if (streamResult is com.nexus.iptv.domain.model.Result.Error) {
    ...
    private suspend fun syncXtreamLiveCatalog(
        ...
        syncProgressBus.emit(
            com.nexus.iptv.domain.sync.SyncProgress(
                section = com.nexus.iptv.domain.sync.Section.LIVE,
                current = 0,
                total = 0,
                currentLabel = "",
                itemsIndexed = 0
    ...
    private suspend fun loadStalkerChannelsByCategory(
        api: StalkerProvider,
        categories: List<com.nexus.iptv.domain.model.Category>,
        onProgress: ((String) -> Unit)?
    ): List<Channel> {
    ...
    private suspend fun syncStalkerLiveCatalogStaged(
        ...
        val preferredCategories = when (val categoriesResult = api.getLiveCategories()) {
            is com.nexus.iptv.domain.model.Result.Success -> categoriesResult.data
                .map { it.toEntity(provider.id) }
                .filterNot { category -> category.categoryId in hiddenLiveCategoryIds }
            is com.nexus.iptv.domain.model.Result.Error -> {
                Log.w(
                    TAG,
            ...
            is com.nexus.iptv.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
        }
        ...
        try {
            when (val streamResult = api.streamLiveStreams { channel ->
                ...
                is com.nexus.iptv.domain.model.Result.Success -> flushBatch()
                is com.nexus.iptv.domain.model.Result.Error -> {
    ...
    private suspend fun stalkerCatalogAccessDiagnostic(
        ...
        val profile = when (val profileResult = api.getAccountProfile()) {
            is com.nexus.iptv.domain.model.Result.Success -> profileResult.data
            else -> return null
        }
    ...
    private fun <T> requireResult(result: com.nexus.iptv.domain.model.Result<T>, fallbackMessage: String): T {
        return when (result) {
            is com.nexus.iptv.domain.model.Result.Success -> result.data
            is com.nexus.iptv.domain.model.Result.Error -> throw IllegalStateException(result.message.ifBlank { fallbackMessage }, result.exception)
            is com.nexus.iptv.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
        }
    }
